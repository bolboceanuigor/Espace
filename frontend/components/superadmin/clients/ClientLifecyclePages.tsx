...
export function ClientDetailPage({ id, tab }: { id: string; tab?: 'activity' | 'tasks' | 'follow-ups' | 'calendar' | 'health' | 'knowledge' | 'notes' | 'files' | 'contacts' | 'decisions' | 'known-issues' | 'links' | 'onboarding' | 'subscription' | 'risk' | 'retention' | 'renewal' | 'churn-risk' }) {
  const path = useLocalizedPath();
  ...
  return (
    <Shell>
      ...
      <nav className="flex flex-wrap gap-2 rounded-lg border border-slate-200 bg-white p-3 text-sm">
        {['activity', 'tasks', 'follow-ups', 'calendar', 'health', 'retention', 'renewal', 'churn-risk', 'knowledge', 'notes', 'files', 'contacts', 'decisions', 'known-issues', 'links', 'onboarding', 'subscription', 'risk'].map((item) => <Link key={item} href={path(`/superadmin/clients/${id}/${item}`)} className={`rounded-md px-3 py-2 ${tab === item ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-700'}`}>{item}</Link>)}
      </nav>
