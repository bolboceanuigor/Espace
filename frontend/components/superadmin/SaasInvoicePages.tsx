...
export function SaasInvoiceDetailPage({ id, print = false }: { id: string; print?: boolean }) {
  ...
  return (
    <div className="space-y-5 bg-slate-50">
      <Header title={invoice.invoiceNumber} subtitle="Detalii factură SaaS, linii și timeline." actions={<><ButtonLink href={localizedPath('/superadmin/billing/saas-invoices')} variant="secondary"><ArrowLeft className="h-4 w-4" /> Înapoi</ButtonLink><ButtonLink href={localizedPath(`/superadmin/billing/saas-invoices/${id}/collections`)} variant="secondary"><FileText className="h-4 w-4" /> Collections</ButtonLink><ButtonLink href={localizedPath(`/superadmin/billing/saas-invoices/${id}/print`)} variant="secondary"><Printer className="h-4 w-4" /> Print</ButtonLink>{Number(invoice.paidAmount || 0) > 0 ? <ButtonLink href={localizedPath(`/superadmin/billing/saas-invoices/${id}/receipt/print`)} variant="secondary"><Receipt className="h-4 w-4" /> Receipt</ButtonLink> : null}</>} />
