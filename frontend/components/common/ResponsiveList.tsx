  ...
  return (
    <>
      <div className="space-y-2 md:hidden">
        {items.map((item) => (
          <div key={keyExtractor(item)} className="rounded-xl border border-border/75 bg-card p-3 shadow-card">
            {renderCard(item)}
          </div>
        ))}
      </div>
      ...
      {renderDesktopHeader && renderDesktopRow ? (
        <div className="hidden rounded-xl border border-border/75 bg-card p-4 shadow-card md:block">
