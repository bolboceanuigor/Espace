    ...
    <div className="space-y-5 pb-8">
      ...
      {data.knowledge ? (
        <section className="grid gap-4 lg:grid-cols-2">
          <Card><h2 className="font-bold text-slate-950">Knowledge highlights</h2><div className="mt-3 space-y-2 text-sm">{[...(data.knowledge.pinnedNotes || []), ...(data.knowledge.recentDecisions || [])].slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl border border-slate-200 px-3 py-2"><b>{item.title}</b><p className="text-slate-500">{item.summary || item.description || item.content || '-'}</p></div>)}{!(data.knowledge.pinnedNotes || []).length && !(data.knowledge.recentDecisions || []).length ? <p className="text-slate-500">Nu exista informatii pinned sau decizii recente.</p> : null}</div></Card>
          <Card><h2 className="font-bold text-slate-950">Contacts & issues</h2><div className="mt-3 space-y-2 text-sm">{(data.knowledge.primaryContacts || []).map((item: any) => <div key={item.id} className="rounded-2xl border border-slate-200 px-3 py-2"><b>{item.fullName}</b><p className="text-slate-500">{item.role || '-'} · {item.phone || item.email || '-'}</p></div>)}{(data.knowledge.openIssues || []).map((item: any) => <div key={item.id} className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2"><b>{item.title}</b><p className="text-amber-700">{item.severity} · {item.status}</p></div>)}{!(data.knowledge.primaryContacts || []).length && !(data.knowledge.openIssues || []).length ? <p className="text-slate-500">Nu exista contacte principale sau probleme deschise.</p> : null}</div></Card>
        </section>
      ) : null}
      <section className="grid gap-4 lg:grid-cols-2">
