  ...
  if (variant === 'table') {
    return (
      <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-card">
        <div className="h-11 border-b border-border/70 bg-muted/45" />
        <div className="divide-y divide-border/50">
          {Array.from({ length: rows }).map((_, index) => (
            <div key={index} className="grid grid-cols-4 gap-4 px-5 py-4">
              <span className="h-4 animate-pulse rounded-full bg-muted" />
              <span className="h-4 animate-pulse rounded-full bg-muted" />
              <span className="h-4 animate-pulse rounded-full bg-muted" />
              <span className="h-4 animate-pulse rounded-full bg-muted" />
            </div>
  ...
  return (
    <div className="space-y-3" aria-live="polite" aria-busy="true">
      ...
      <div className={`grid gap-3 ${variant === 'page' ? 'md:grid-cols-3' : ''}`}>
        {Array.from({ length: rows }).map((_, index) => (
          <div key={index} className="h-28 animate-pulse rounded-2xl border border-border/70 bg-card shadow-card">
