...
const tableWrapper = 'overflow-x-auto rounded-2xl border border-border/80 bg-card shadow-card';
...
export function TableHead({ className = '', ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return (
    <thead
      className={`bg-card text-left text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground ${className}`.trim()}
      {...props}
    />
  );
}
...
export function TableRow({
  ...
  return (
    <tr
      className={`${hover ? 'transition hover:bg-accent/30' : ''} ${className}`.trim()}
      {...props}
    />
  );
}
