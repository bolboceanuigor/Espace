...
export default function AdminInvoicePrintPage() {
  ...
  useEffect(() => {
    if (!invoiceId) return;
    let active = true;
    Promise.all([invoicesApi.get(invoiceId), loadAdminApcOrganization()])
  ...
  const amount = Number(invoice?.amount ?? invoice?.finalAmount ?? invoice?.totalDue ?? 0);
  const paidAmount = Number(invoice?.paidAmount ?? payments.reduce((sum: number, payment: any) => sum + Number(payment.amount || 0), 0));
  const remainingAmount = Number(invoice?.remainingDebt ?? invoice?.remainingAmount ?? Math.max(amount - paidAmount, 0));
  ...
  return (
    ...
    >
      <PrintActions printLabel="Printează factura" backHref={`/admin/invoices/${invoiceId}`} />

      {state === 'loading' ? <p className="rounded-xl border border-slate-200 p-4 text-sm text-slate-600">Se încarcă documentul...</p> : null}
      {state === 'error' ? <p className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700">Factura nu a fost găsită.</p> : null}
      ...
      {state === 'ready' && invoice ? (
        <>
          <PrintSection title="Date factură">
            <PrintInfoGrid
              rows={[
                ...
                ['Bloc', apartment?.building?.name || invoice.building?.name || '-'],
          ...
          <PrintSection title="Sume">
            <PrintInfoGrid
              rows={[
                ['Suma totală', formatMdlPrint(amount)],
                ['Achitat', formatMdlPrint(paidAmount)],
                ['Restant', formatMdlPrint(remainingAmount)],
              ]}
            />
          </PrintSection>
