...
export default function SuperadminOrganizationSubscriptionPage() {
  ...
  useEffect(() => {
    load().catch(() => {
      setSource('mock');
      setError('API indisponibil temporar. Sunt afișate date demo.');
    });
  }, [load]);
      ...
      <PageHeader
        ...
        rightSlot={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href={`/ro/superadmin/organizations/${orgId}`} className="rounded-2xl border border-border/70 px-4 py-2 text-sm font-semibold text-foreground hover:bg-muted/60">
              Înapoi la asociație
            </Link>
          </div>
        }
